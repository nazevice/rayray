...
const LectureCard = ({ lecture, handleOpen, handleDelete }) => {
  ...
  <CardContent sx={{flexGrow: 1}}>
    ...
    maxWidth: '350px',}}variant="h5">{lecture.lectureName}</Typography>
    <Typography sx={{textOverflow: 'ellipsis', overflow: 'hidden',whiteSpace: 'nowrap',
    maxWidth: '350px', fontSize: "14"}} gutterBottom>{lecture.modulName}</Typography>
      <Typography sx={{textOverflow: 'ellipsis', overflow: 'hidden',whiteSpace: 'nowrap',
    maxWidth: '350px', fontSize: "14"}} color="text.secondary" gutterBottom>{lecture.duration + " Minuten"}</Typography>
    </Box>
  </CardContent>
